...
@Injectable()
export class ProductManagerCounselingService {
  private readonly logger = new Logger(ProductManagerCounselingService.name);

  constructor(
    @InjectRepository(PMCounselingTimes)
    private pmCounselingTimesRepository: Repository<PMCounselingTimes>,
    @InjectRepository(PMTimeSlot)
    private pmTimeSlotRepository: Repository<PMTimeSlot>,
  ...
  async createWeekdaysAndTimeSlots() {
    ...
    const existingDays = await this.pmCounselingTimesRepository.find({
      where: {
        date: Between(
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (!existingDatesSet.has(formattedDate)) {
        ...
        const pmCounselingTime = new PMCounselingTimes();
        pmCounselingTime.id = uuidv4();
        pmCounselingTime.day = dayOfWeek;
        pmCounselingTime.date = formattedDate;

        await this.pmCounselingTimesRepository.save(pmCounselingTime);

        for (const hour of hours) {
          const timeSlot = new PMTimeSlot();

          timeSlot.id = uuidv4();
          timeSlot.clock = hour;
          timeSlot.booked = false;
          timeSlot.active = false;
          timeSlot.pmCounselingTimes = pmCounselingTime;

          await this.pmTimeSlotRepository.save(timeSlot);
        }
  ...
  async getWeekWithTimeSlots() {
    ...
    const [data, total] = await this.pmCounselingTimesRepository.findAndCount({
      relations: ['pmTimeSlots', 'pmTimeSlots.user'], // ✅ Ensure time slots are included
    });
    ...
    const responseData = filteredData.map((item) => ({
      ...
      day: item.day,
      date: item.date,
      pmTimeSlots: item.pmTimeSlots.map((slot) => ({
        id: slot.id,
        active: slot.active,
        booked: slot.booked,
        clock: slot.clock,
  ...
  async deleteAllDaysWithTimeSlots() {
    await this.pmTimeSlotRepository.delete({});
    await this.pmCounselingTimesRepository.delete({});
  ...
  async getSlotTimeWithDayAndUser(slotTimeId: string) {
    try {
      ...
      const slotTime = await this.pmTimeSlotRepository.findOne({
        where: { id: slotTimeId },
        relations: ['pmCounselingTimes', 'user'],
      });
      ...
      return {
        ...
        user: slotTime.user
          ...
          : null,
        pmCounselingTimes: {
          id: slotTime.pmCounselingTimes.id,
          day: slotTime.pmCounselingTimes.day,
          date: slotTime.pmCounselingTimes.date,
        },
      };
    } catch (error) {
  ...
  async updateActiveStatus(updateActiveDto: UpdateActiveDto, user: User) {
    try {
      ...
      const timeSlot = await this.pmTimeSlotRepository.findOne({
