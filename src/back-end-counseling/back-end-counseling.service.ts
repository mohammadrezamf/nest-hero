...
@Injectable()
export class BackEndCounselingService {
  private readonly logger = new Logger(BackEndCounselingService.name);

  constructor(
    @InjectRepository(BackEndCounselingTimes)
    private backEndCounselingTimesRepo: Repository<BackEndCounselingTimes>,
    @InjectRepository(BackEndTimeSlot)
    private backEndTimeSlotRepo: Repository<BackEndTimeSlot>,
  ...
  async createWeekdaysAndTimeSlots() {
    ...
    const existingDays = await this.backEndCounselingTimesRepo.find({
      where: {
        date: Between(
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (!existingDatesSet.has(formattedDate)) {
        ...
        const backEndCounselingTime = new BackEndCounselingTimes();
        backEndCounselingTime.id = uuidv4();
        backEndCounselingTime.day = dayOfWeek;
        backEndCounselingTime.date = formattedDate;

        await this.backEndCounselingTimesRepo.save(backEndCounselingTime);

        for (const hour of hours) {
          const timeSlot = new BackEndTimeSlot();

          timeSlot.id = uuidv4();
          timeSlot.clock = hour;
          timeSlot.booked = false;
          timeSlot.active = false;
          timeSlot.backEndCounselingTimes = backEndCounselingTime;

          await this.backEndTimeSlotRepo.save(timeSlot);
        }
  ...
  async getWeekWithTimeSlots() {
    ...
    const [data, total] = await this.backEndCounselingTimesRepo.findAndCount({
      relations: ['backEndTimeSlots', 'backEndTimeSlots.user'], // ✅ Ensure time slots are included
    });
  ...
  async deleteAllDaysWithTimeSlots() {
    await this.backEndTimeSlotRepo.delete({});
    await this.backEndCounselingTimesRepo.delete({});
  ...
  async getSlotTimeWithDayAndUser(slotTimeId: string) {
    try {
      ...
      const slotTime = await this.backEndTimeSlotRepo.findOne({
        where: { id: slotTimeId },
        relations: ['backEndCounselingTimes', 'user'],
      });
      ...
      return {
        ...
        user: slotTime.user
          ...
          : null,
        backEndCounselingTimes: {
          id: slotTime.backEndCounselingTimes.id,
          day: slotTime.backEndCounselingTimes.day,
          date: slotTime.backEndCounselingTimes.date,
        },
      };
    } catch (error) {
  ...
  async updateActiveStatus(updateActiveDto: UpdateActiveDto, user: User) {
    try {
      ...
      const timeSlot = await this.backEndTimeSlotRepo.findOne({
  ...
  async updateBooked(updateBookedDto: UpdateBookedDto, user: User) {
    try {
      if (user.role === UserRole.BACK_END) {
        return new BadRequestException(
          `You do not have permission to perform this action`,
        );
      }
